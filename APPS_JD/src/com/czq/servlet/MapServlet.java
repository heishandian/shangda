...
import com.czq.entitiy.MapInfo;
...
public class MapServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		jsonArray = JSONArray.fromObject(getMapInfo());
	...
	private ArrayList<MapInfo> getMapInfo() {// 预警查询
		...
		String sql1 = "SELECT x.sewageID , x.short_title, y.name,x.coordinateX, x.coordinateY,4 AS isabnormal from sewage x , area y WHERE x.areaID=y.id";
		// 断电断线
		String sql2 = "SELECT sewageID ,short_title,coordinateX,coordinateY ,3 AS isabnormal FROM sewage WHERE sewageID in(((SELECT DISTINCT sewageID FROM sewage) EXCEPT (SELECT DISTINCT sewageID  FROM run_data  where CONVERT(varchar(12),testingtime,120) = CONVERT(varchar(12),getdate(),120))) )";
		// 设备异常
		String sql3 = "SELECT  sewageID , short_title, coordinateX,coordinateY,2 AS isabnormal FROM sewage WHERE sewageID in (SELECT sewageID FROM run_data_abnormal WHERE CONVERT(varchar(12),testingtime,120) = CONVERT(varchar(12),getdate(),120)) ";
		// 水质异常
		String sql4 = "SELECT  sewageID,short_title,coordinateX,coordinateY,1 AS isabnormal FROM sewage  WHERE sewageID in (SELECT sewageID FROM detection_data_abnormal WHERE CONVERT(varchar(12),testingtime,120) = CONVERT(varchar(12),getdate(),120) )";
		...
		try {
			...
			if (resultSet1 != null) {
				while (resultSet1.next()) {
					...
					temp.setCoordinateX(resultSet1.getFloat(4));
					temp.setCoordinateY(resultSet1.getFloat(5));
			...
			if (resultSet2 != null) {
				while (resultSet2.next()) {
					...
					temp.setCoordinateX(resultSet2.getFloat(3));
					temp.setCoordinateY(resultSet2.getFloat(4));
			...
			if (resultSet3 != null) {
				while (resultSet3.next()) {
					...
					temp.setCoordinateX(resultSet3.getFloat(3));
					temp.setCoordinateY(resultSet3.getFloat(4));
			...
			if (resultSet4 != null) {
				while (resultSet4.next()) {
					...
					temp.setCoordinateX(resultSet4.getFloat(3));
					temp.setCoordinateY(resultSet4.getFloat(4));
